...
| Поле | Пример |
|-----|--------|
| **Заголовок** | Выбор точки выдачи заказа |
| **Акторы** | Клиент |
| **Предусловие** | Клиент авторизован в приложении кофейни |
| **Ограничения** | 1. Часы работы точек<br>2. Географическая доступность |
| **Триггер** | Клиент нажимает кнопку «Выбрать точку выдачи» |
| **Основной сценарий** | 1. Клиент открывает экран выбора точки выдачи<br>2. Система отображает список доступных точек<br>3. Клиент выбирает точку<br>4. Система сохраняет выбор и подтверждает действие |
| **Альтернативный сценарий** | 1. Нет доступных точек поблизости<br>2. Точка временно закрыта |
| **Результат** | Выбранная точка выдачи сохранена в заказе |
